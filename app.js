...
(function () {
  ...
  // the recipient sees the sender's light. The nudge is deterministic from lj: 0 = none.
  function applyLightPreset(i, jitterSeed) {
    if (!window.CakeLook) return;
    var P = LIGHT_PRESETS[Math.max(0, Math.min(LIGHT_PRESETS.length - 1, i | 0))], LK = CakeLook.LOOK, j = jitterSeed ? 1 : 0;
    lightPreset = i | 0; lightJitter = jitterSeed | 0;
    function r(k) { var n = Math.sin((jitterSeed | 0) * 127.1 + k * 311.7) * 43758.5453; return n - Math.floor(n); }
    LK.keyScale = +(P.keyScale + j * (r(1) - 0.5) * 0.2).toFixed(2);
    LK.keyDir.elevation = Math.round(Math.max(8, Math.min(85, P.elevation + j * (r(2) - 0.5) * 12)));
  ...
  function applyLightFromConfig(cfg) {                  // called by build(): the light is part of the cake
    ...
    lightApplied = true; applyLightPreset(cfg.lp, cfg.lj); updateRoomLights();
  }
  ...
  function applyRandomLook(d, onLoad) {
    ...
    d.lp = B.lights[randInt(0, B.lights.length - 1)]; d.lj = randInt(1, 99);   // the light is part of the cake (v1.14)
  ...
  // ---- v1.11: the ribbon (its widths, height, hand and cloth) lives in ribbon.js ----
  var RIBBONS = CakeRibbon.create({ PALETTES: PALETTES, clampIndex: clampIndex, baseFillet: function () { return FONDANT_BASE_FILLET; } });
  var RIBBON = RIBBONS.RIBBON;
  ...
  // ---- v1.11: the sponge's layers and cut faces live in sponge.js ----
  var SPONGING = CakeSponge.create({ FILL_T: function () { return FILL_T; }, spongeColour: function () { return SPONGE; }, hexCss: function (h) { return hexCss(h); } });
  function layerScheme(tierH, spongeLayers) { return SPONGING.layerScheme(tierH, spongeLayers); }
  function paintLayers(g, W, H, layers, scheme, span) { return SPONGING.paintLayers(g, W, H, layers, scheme, span); }
  function makeCutFaceTexture(layers, scheme, tier, frostingHex, thick) { return SPONGING.makeCutFaceTexture(layers, scheme, tier, frostingHex, thick); }
  ...
  // ---- v1.12: a tier's materials, a naked tier's stack, and the wedge live in body.js ----
  var BODY = CakeBody.create({
    ...
    CAP_H: function () { return CAP_H; }, NAKED_CAP_H: function () { return NAKED_CAP_H; }, FONDANT_BASE_FILLET: function () { return FONDANT_BASE_FILLET; },
    SPONGE: function () { return SPONGE; }, SPONGE_CRUMB: function () { return SPONGE_CRUMB; }, SPONGE_WOBBLE: function () { return SPONGE_WOBBLE; },
    CYL_SEG: function () { return CYL_SEG; }, WEDGES_PER_TIER: function () { return WEDGES_PER_TIER; }, INK_DARK: INK_DARK, INK_LIGHT: INK_LIGHT
  });
  var CAP_H = 0.32;
  ...
  // ---- v1.11: tier sizes and positions live in tiers.js ----
  var TIERING = CakeTiers.create({ classicShape: classicShape, stepToR: stepToR, stepToH: stepToH,
    frostingHasThickness: function (cfg, i) { return frostingHasThickness(cfg, i); },
    FROST_T: function () { return FROST_T; }, FROST_TOP: function () { return FROST_TOP; }, PLATE_TOP: function () { return PLATE_TOP; } });
  function tiersFor(cfg) { return TIERING.tiersFor(cfg); }
  function tierTops(cfg) { return TIERING.tierTops(cfg); }
  ...
  // =====================================================================
  // ---- v1.18: easing and tweens live in animate.js ----
  var EASE = CakeAnimate.EASE, tweens = CakeAnimate.tweens;
  function tween(o) { return CakeAnimate.tween(o); }
  function finishTweens() {
    var all = tweens.slice(); CakeAnimate.clear();
    all.forEach(function (o) { o.update(1, 1); if (o.done) o.done(); });
  }
  function killTweens(tag) { CakeAnimate.clear(tag); }
  ...
  // ---- v1.13: the decor's parts live in candles.js, flames.js and sparklers.js ----
  var CANDLES = CakeCandles.create();
  ...
  var SPARKLERS = CakeSparklers.create({ light: function () { return sparkLight; }, darkness: function () { return darkness; },
                                        parent: function () { return built; }, studioEnv: function () { return getStudioEnv(); } });
  var sparklers = SPARKLERS.list;
  ...
  // the ribbon, and the surface candles may stand on. B is the build's shared context (v1.21).
  function buildTier(B, tier, i) {
    var isTop = (i === B.tiers.length - 1);
    tier.idx = i;
    ...
    var open = B.cfg.cutaway ? Math.PI / 4 : 0;          // dev wedge removed
    ...
    if (tier === B.messageTier && B.cfg.m && showMessage) {
      ...
      var keptMap = (B.opts && B.opts.keepMessage && B.keepMessageMap) ? B.keepMessageMap : null;
      sideMat = new THREE.MeshStandardMaterial({
        color: 0xffffff, roughness: TM.naked ? 0.95 : ((!TM.fdOn && TM.style === 4) ? 0.85 : 0.62),
        map: keptMap || makeMessageTexture(B.cfg.m, ink, frosting, rr, TM.fdOn ? bodyH : tier.hs, msgBase), vertexColors: true
      });
      ...
      else if (!TM.fdOn && window.CakeFrosting) { var bm = CakeFrosting.spongeMaps(B.cfg.rk); bm.noAlbedo = true; CakeFrosting.dressFondant(sideMat, bm, tier.rs); }   // …or on the baked crust
      BODY.nightGlow(sideMat, 0xffffff, true);
    }
    var body;
    if (!TM.fdOn) {
      ...
      body = BODY.buildStack(B.cfg, tier, TM, sideMat, 0, Math.PI * 2, CYL_SEG, false);
      body.position.y = B.y;
      tg.add(body);
      if (tier === B.messageTier) { messageMesh = body; body.__tier = tier; body.__bodyH = tier.hs; body.__stack = true; }
    } else {
      ...
      var above = B.tiers[i + 1];                 // the tier sitting on this one, if any
      ...
      body.position.y = B.y;
      tg.add(body);
      if (tier === B.messageTier) { messageMesh = body; body.__tier = tier; body.__bodyH = bodyH; }
    }
    ...
    var rt = B.cfg.rt[i];
    if (rt && rt.on) {
      var rw = RIBBONS.ribbonWidth(rt.w);
      // Hugs the fondant's surface, or bridges a naked stack's fillings taut (no tucking in).
      var rbY = RIBBONS.ribbonY(TM, tier, rw, rt.p), hand = RIBBONS.ribbonHandFor(B.cfg, tier, TM, rw, rt);
      var rbR = RIBBONS.ribbonRadius(TM, rr, bodyH, pOpts, rbY, rw);
    ...
    var topY = B.y + tier.h;
    if (isTop) {
      B.surfaces.push({ y: topY, rMax: tier.r - 0.25, rMin: 0 });
    } else {
      // Exposed annulus around the next tier up
      B.surfaces.push({ y: topY, rMax: tier.r - 0.22, rMin: B.tiers[i + 1].r + 0.22 });
    }
    B.y = topY;
  }
  ...
  function build(cfg, opts) {
    ...
    clearGroup(built);
    flames.length = 0;         // v1.20: emptied in place — modules hold these arrays
    wicks.length = 0;
    messageMesh = null;
    tierGroups = [];
    D.spawn = null;
    ...
    var B = { cfg: cfg, opts: opts, keepMessageMap: keepMessageMap, tiers: tiers, localShared: localShared, y: y, surfaces: surfaces, messageTier: messageTier };
    tiers.forEach(function (tier, i) { buildTier(B, tier, i); });
    y = B.y;
  ...
  function updateMessage(m) {
    ...
    var mat;
    if (m && showMessage) {
      var TMm = BODY.tierMaterials(config, tier);              // the message tier's own layers
  ...
  var lastMessageSpan = null;                            // where the writing is (message.js fills it in), so sprinkles keep clear
  function placeSprinkles(cfg, tiers) { SPRINKLES.place(cfg, tiers); }
  function updateSprinkleLights() { SPRINKLES.updateLights(); }
  CakeMaterials.warm('candles', function () {
    var g = new THREE.Group(), white = new THREE.Color(1, 1, 1);
    [CANDLES.styleMaterial(0, 0x4FC3F7), CANDLES.styleMaterial(2, 0x4FC3F7), CANDLES.styleMaterial(4, 0)].forEach(function (m) {   // wax, patterned wax, metal — instanced, WITH per-instance colour (a different program without it)
  ...
  CakeMaterials.warm('number-candles', function () {
    ...
    var g = new THREE.Group(); g.add(new THREE.Mesh(dg.geo, CANDLES.makeWaxMaterial(0x4FC3F7))); g.add(new THREE.Mesh(numberSpikeGeo, holderMat)); return g;
  });
  ...
  //  Colour helpers
  // =====================================================================
  // ---- v1.10: the writing on the cake lives in message.js ----
  function makeMessageTexture(text, ink, frostingHex, radius, bodyH, base, ownCanvas) {
    var r = CakeMessage.texture(text, ink, frostingHex, radius, bodyH, base, ownCanvas);
    lastMessageSpan = CakeMessage.lastSpan();
    return r;
  }
  ...
  function updateBleed() {
    ...
    CakeLook.setBleed(scene, new THREE.Color(frosting), r, onFloor && left > 0 && V.mode !== 'slice', roomLit, candleLight.intensity);
  }
  ...
  function frame() {
    ...
    var frameStart = now;
    CakeAnimate.update(now);
    PLACE.updateSpawn(now);
    ...
    if (spinEnabled) SPINNER.update(now, dt);
    // Tilt and twist are held, not sprung, and apply whether or not the spin loop runs.
    camElev = Math.max(CAM_ELEV_MIN, Math.min(CAM_ELEV_MAX, CAM_ELEV_BASE + S.tiltX * 180 / Math.PI));
    camRoll = S.bankZ;
    frameCamera();
    updateShadow();
    ...
    SPARKLERS.update(t);                                  // they keep going when the candles are blown out
  ...
  // =====================================================================
  // ---- v1.18: the smoke from a blown-out wick lives in smoke.js ----
  var SMOKE = CakeSmoke.create({ parent: cakeGroup, EASE: EASE });
  var smokeTex = SMOKE.texture, smokes = SMOKE.list;
  function puffSmoke(f, dirX, dirZ) { return SMOKE.puff(f, dirX, dirZ); }
  ...
  // ---- v1.19: the spin's state, in one object (it was eight separate variables) ----
  var S = { omega: SPIN.idle,   // free-spin angular velocity, rad/s (momentum)
            dragW: 0,           // the thumb's own angular velocity right now, rad/s
            tiltX: 0,           // held tilt (X), radians, clamped to TILT
            bankZ: 0,           // held twist (Z), radians, clamped to TWIST — two-finger gesture
            dragging: false,
            spinFree: false,    // true once the user has taken hold of the cake
            heldOmega: 0,       // the speed the cake had when it was caught (for building speed)
            idleResumeAt: 0 };  // after a catch, the ambient turn waits until this time
  ...
  // v1.19: reset IN PLACE. mic.js, spin.js and blowout.js hold this object; replacing it (as the
  // two resets used to) left them writing to the old one — in v1.18 the microphone's level went
  // nowhere after the first reset, and a fast spin stopped blowing the candles out.
  function resetBlow(v) { for (var k in blow) delete blow[k]; for (var j in v) blow[j] = v[j]; }
  // ---- v1.19: blowing the candles out lives in blowout.js; the spin's physics in spin.js ----
  var BLOWOUT = CakeBlowout.create({ flames: function () { return flames; }, blow: blow, camera: camera,
    tween: function (o) { return tween(o); }, EASE: EASE, puffSmoke: function (f, x, z) { return puffSmoke(f, x, z); },
    litCount: function () { return litCount(); }, reveal: function () { return reveal(); } });
  function wave(strength, dirX, dirZ) { return BLOWOUT.wave(strength, dirX, dirZ); }
  function setLean(sideways, away) { return BLOWOUT.setLean(sideways, away); }
  var SPINNER = CakeSpin.create({ S: S, SPIN: SPIN, blow: blow, wave: wave, setLean: setLean,
    reduceMotion: function () { return reduceMotion; },
    getAzimuth: function () { return camAzimuth; }, setAzimuth: function (v) { camAzimuth = v; },
    getLean: function () { return leanNow; }, setLeanNow: function (v) { leanNow = v; } });
  ...
  (function () {
    ...
    canvas.addEventListener('pointerdown', function (e) {
      ...
      if (primary === null) {
        ...
        samples.length = 0;
        S.dragging = true; S.spinFree = true; S.dragW = 0;
    ...
    canvas.addEventListener('pointermove', function (e) {
      ...
      if (count() >= 2) {
        ...
        if (TWIST.enabled && ang !== null && pinchPrev !== null) {
          ...
          S.bankZ = Math.max(TWIST.min, Math.min(TWIST.max, S.bankZ + d));
        }
        if (sp !== null && spanPrev) {
      ...
      pushSample(w, now);
      S.dragW = isFinite(w) ? w : 0;

      // Drag down to look further over the top, up to see more of the side.
      S.tiltX = Math.max(TILT.min, Math.min(TILT.max, S.tiltX + dy / TILT.pxPerRad));
      lastT = now;
    });
    ...
    function release(e) {
      ...
      if (Math.abs(v) < 0.15) { S.omega = 0; S.idleResumeAt = now + SPIN.catchIdleDelay; }
      else if (S.heldOmega && Math.sign(v) === Math.sign(S.heldOmega)) S.omega = S.heldOmega + v;
      else S.omega = v;
      S.omega = Math.max(-SPIN.max, Math.min(SPIN.max, S.omega));
      S.heldOmega = 0;
      S.dragW = 0;
      samples.length = 0;
    }
  ...
  // Microphone
  // ---- v1.18: the microphone (listening for a breath) lives in mic.js ----
  var MIC = CakeMic.create({ blow: blow });
  ...
  // static draw call.
  // ---- v1.15: confetti lives in confetti.js ----
  var CONF = CakeConfetti.create({ scene: scene, camera: camera, tiersFor: function (cfg) { return tiersFor(cfg); }, PLATE_TOP: function () { return PLATE_TOP; } });
  var confetti = CONF.mesh, confettiPieces = CONF.pieces;
  ...
  function showRevealedControls() {
    if (V.mode === 'slice') { showSliceDone(); return; }
  ...
  function relight() {
    if (V.cut) { while (V.group.children.length) V.group.remove(V.group.children[0]); V.cut = null; built.visible = true; }
    blow.revealed = false;
  ...
  // ---- v1.17: the viewer's state, in one object (the cut, the slice page, which page we're on) ----
  var V = { cut: null,          // { wedges: [ {mesh group, tier, i, gone} ], plate, lifted, code, sentBack }
            slice: null,        // slice-page state
            mode: 'cake',       // 'cake' | 'slice'
            group: new THREE.Group() };   // the wedges live here while the cake is cut
  cakeGroup.add(V.group);
  ...
  function startCut() {
    ...
    V.cut = { wedges: [], plate: null, lifted: null, code: code, sentBack: !!st.sentBack, total: 0 };
    var tiers = tierTops(cfg);
    var TMs = tiers.map(function (tier) { return BODY.tierMaterials(cfg, tier); });   // one set per tier, shared by its wedges
    ...
    V.cut.total = V.cut.wedges.length;
    V.cut.tiers = tiers.length;
    V.cut.seams = new THREE.Group(); V.group.add(V.cut.seams);
    V.cut.seamMat = new THREE.MeshBasicMaterial({ color: 0x000000, transparent: true, opacity: 0.16, depthWrite: false });
  ...
  // ---- v1.17: the cut's geometry and bookkeeping live in cut.js; the viewer's UI for it stays here ----
  var CUT = CakeCut.create({ V: V, config: function () { return config; }, tierTops: function (c) { return tierTops(c); }, WEDGES_PER_TIER: function () { return WEDGES_PER_TIER; } });
  var PLATE = CUT.PLATE;
  function wobbleTier(ti) {
    V.cut.wedges.forEach(function (w) {
  ...
  function wedgeAt(clientX, clientY) {
    ...
    var hits = _ray.intersectObjects(V.group.children, true);
    var top = CUT.topRemainingTier();
  ...
  function sendLiftedSlice(name, back) {
    ...
    tween({ duration: 280, ease: EASE.soft, update: function (k) {
      ...
      if (plate) { V.group.remove(plate); }
      V.cut.plate = null; V.cut.lifted = null;
      updateCutUI();
    } });
    return url;
  }
  ...
  function showFinished() {
    var st = loadCutState(V.cut.code), names = st.names || [];
  ...
  function enterSlice(cfg, sl) {
    ...
    holder.add(w);
    var off = CUT.wedgeOffsetFor(tier, wi, 0, 0);
    ...
    CONF.setLandings([{ y: topY, r: tier.r * 0.8, rInner: 0 }]);
    fitShadow(tier.r * 0.8);
    resetBlow({ enabled: false, lastWave: 0, micLevel: 0, micHold: 0, total: 0, revealed: true });
    setFrame('cake', 0.9, true);
    camAzimuth = 0; spinEnabled = true; S.spinFree = false;
    // Chrome
  ...
  function showSliceDone() {
    var isBack = V.slice && V.slice.back && config.from;
  ...
  // ---- v1.16: the builder's config lives in the store; `draft` is a live reference to it ----
  var STORE = CakeStore.create({ normalize: normalize, merge: CakeSchema.merge });
  var draft = null;          // builder state: STORE.get(), refreshed by the subscription below
  STORE.subscribe(function (cfg, opts) { draft = cfg; build(cfg, opts.showMessage !== undefined ? { showMessage: opts.showMessage } : undefined); });
  var msgTimer = null;
  ...
  function refreshAutoSwatch() {
    ...
    if (els.swTc) {
      ...
      if (tcEl) tcEl.style.background = BODY.pickInk(f, 0);
    }
  }
  ...
  function scheduleBuild(opts) {
    ...
    buildPending = true;
    requestAnimationFrame(function () { buildPending = false; var o = buildOpts; buildOpts = null; STORE.commit(o || undefined); });
  }
  function syncRibbon() {
  ...
  function wireBuilder() {
    ...
    Array.prototype.forEach.call($('bake-rack').children, function (b) {
      b.addEventListener('click', function () { draft.rk = +b.getAttribute('data-rk'); draft = STORE.set(draft, { silent: true }); syncFrosting(); STORE.commit(); });
    });
    ...
    Array.prototype.forEach.call($('frost-style').children, function (b) {
      b.addEventListener('click', function () {
        ...
        draft = STORE.set(draft, { silent: true });
        syncFrosting(); updateColourNote(); STORE.commit();
      });
    });
    Array.prototype.forEach.call($('layers').children, function (b) {
      b.addEventListener('click', function () { draft.ly = +b.getAttribute('data-ly'); syncFrosting(); STORE.commit(); });
    });
    ...
    $('f-sw').addEventListener('input', function (e) {
      draft.sh[curTier].r = +e.target.value; draft = STORE.set(draft, { silent: true });   // normalize re-clamps the tiers above
      syncShape(); syncRibbon(); scheduleBuild({ keepMessage: true });
    });
    $('f-sh').addEventListener('input', function (e) {
      draft.sh[curTier].h = +e.target.value; draft = STORE.set(draft, { silent: true });
    ...
    $('shape-reset').addEventListener('click', function () {
      draft.sh = classicShape(draft.t); draft = STORE.set(draft, { silent: true });
      syncShape(); STORE.commit();
    });
    $('f-ribbon').addEventListener('change', function (e) {
      ...
      draft = STORE.set(draft, { silent: true });
      syncRibbon(); STORE.commit();
    });
    $('f-ra').addEventListener('input', function (e) {
      draft.rt[curTier].a = +e.target.value; draft = STORE.set(draft, { silent: true });
      syncRibbon(); scheduleBuild();
    });
    Array.prototype.forEach.call($('rib-material').children, function (b) {
      b.addEventListener('click', function () { draft.rm = +b.getAttribute('data-rm'); draft = STORE.set(draft, { silent: true }); syncRibbon(); STORE.commit(); });
    });
    $('f-rp').addEventListener('input', function (e) {
      draft.rt[curTier].p = +e.target.value; draft = STORE.set(draft, { silent: true });
      syncRibbon(); scheduleBuild();
    });
    $('f-rw').addEventListener('input', function (e) {
      draft.rt[curTier].w = +e.target.value; draft = STORE.set(draft, { silent: true });
      syncRibbon(); scheduleBuild();
    });
    ...
    [[0, 'Smooth'], [8, 'Rings'], [9, 'Whirl'], [4, 'Combed'], [7, 'Ridged'], [10, 'Sweep'], [11, 'Broad sweep'], [2, 'Rustic'], [6, 'Deep rustic']].forEach(function (f) {
      ...
      b.addEventListener('click', function () {
        ...
        draft.ff = ff; draft = STORE.set(draft, { silent: true }); syncFrosting(); STORE.commit();   // maps only: same shader
      });
    ...
    LIGHT_PRESETS.forEach(function (P, i) {
      ...
      b.addEventListener('click', function () { draft.lp = i; draft.lj = 0; STORE.commit(); syncFrosting(); });
    ...
    CANDLE_STYLES.concat(['All']).forEach(function (name, i) {
      ...
      b.addEventListener('click', function () { draft.cs = cs; draft = STORE.set(draft, { silent: true }); syncCandleMode(); STORE.commit(); });
      $('candle-styles').appendChild(b);
    });
    Array.prototype.forEach.call($('sparklers').children, function (b) {
      b.addEventListener('click', function () { draft.sk = +b.getAttribute('data-sk'); draft = STORE.set(draft, { silent: true }); syncCandleMode(); STORE.commit(); });
    });
    $('f-sa').addEventListener('input', function (e) { draft.sa = +e.target.value; draft = STORE.set(draft, { silent: true }); syncSprinkles(); scheduleBuild(); });
    ...
    Array.prototype.forEach.call(els.tiers.children, function (b) {
      b.addEventListener('click', function () {
        ...
        draft = STORE.set(draft, { silent: true });                         // re-clamps the stack
  ...
  function wireViewer() {
    ...
    (function () {
      ...
      canvas.addEventListener('pointerup', function (e) {
        if (!down || !V.cut || V.cut.lifted) { down = null; return; }
  ...
  CakeMaterials.warm('cake', function () {
    ...
    BODY.nightGlow(msgMat, 0xffffff, true);                                   // message band: map + emissiveMap
  ...
  function route() {
    ...
    while (V.group.children.length) V.group.remove(V.group.children[0]);
    V.cut = null; V.slice = null; V.mode = 'cake';
    ...
    if (fromLink) {
      ...
      if (pre) { draft = STORE.set(pre, { silent: true }); draft.__prefilled = true; history.replaceState(null, '', location.pathname); }   // consume the pre-fill
      if (!draft) { draft = STORE.set(DEFAULTS, { silent: true }); draft.o = FEATURES.occasion ? OCCASION_UNCHOSEN : OCCASION_DEFAULT; draft.ly = 2; draft.frsty = draft.frsty.map(function () { return 0; }); draft.fds = draft.fds.map(function () { return 0; }); draft.rt.forEach(function (t) { t.on = false; }); draft = STORE.set(applyRandomLook(STORE.set(draft, { silent: true }), true), { silent: true });
  ...
  window.cake = {
    set: function (partial) {                         // v1.16: one path — the store
      if (draft && !document.body.classList.contains('mode-viewer')) { STORE.set(CakeSchema.merge(config, partial), { showMessage: showMessage }); syncForm(); }
      else build(CakeSchema.merge(config, partial), { showMessage: showMessage });
    },
    get config() { return config; },
    set rotate(v) { spinEnabled = !!v; },
    set spin(v) { S.omega = v; },
    get spin() { return S.omega; },
    ...
    set elev(v) { S.tiltX = (v - CAM_ELEV_BASE) * Math.PI / 180; }, get elev() { return camElev; },
    ...
    ao: AO,
    __breathe: function (level) { MIC.simulate(level); },   // the harness: blow through the mic's own path
    ...
    PIXEL: PIXEL,
    cut: function () { return V.cut ? { left: CUT.slicesLeft(), lifted: !!V.cut.lifted, sentBack: V.cut.sentBack, total: V.cut.total } : null; },
    startCut: startCut, liftFirst: function () { if (!V.cut) return; var w = V.cut.wedges.filter(function (x) { return x.visible && x.userData.tier === CUT.topRemainingTier(); })[0]; if (w) liftWedge(w); },
    ...
    confetti: function () { return { count: confetti.count, active: CONF.active(), resting: confettiPieces.filter(function (c) { return c.resting; }).length }; },
    debug: function () { return { azimuth: +camAzimuth.toFixed(2), omega: +S.omega.toFixed(2), elev: +camElev.toFixed(1), roll: +camRoll.toFixed(3), twist: +S.bankZ.toFixed(3), zoom: +camZoom.toFixed(2), dragging: S.dragging, lit: litCount(), state: document.body.getAttribute('data-vstate'), blow: { enabled: blow.enabled, mic: blow.micLevel, hold: blow.micHold } }; },
    blowAll: function () { for (var i = 0; i < flames.length; i++) BLOWOUT.extinguish(flames[i], 0, -1); }
  };
})();
